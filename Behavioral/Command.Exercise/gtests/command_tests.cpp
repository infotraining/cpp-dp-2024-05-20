...
struct UndoableCmdTests : CommandTests
{
    CommandHistory cmd_history;
};

struct ClearCmd_Execute : UndoableCmdTests
{
    ClearCmd clear_cmd;
...
struct AddTextCmd_Execute : UndoableCmdTests
{
...
struct ToUpperCmd_Execute : UndoableCmdTests
{
...
struct PasteCmd_Execute : UndoableCmdTests
{
...
struct MockUndoableCommand : UndoableCommand
{
    MOCK_METHOD0(execute, void());
    MOCK_METHOD0(undo, void());
    MOCK_CONST_METHOD0(clone, UndoableCommandPtr());
};

TEST_F(UndoCmd_Execute, PopsLastCommandFromHistory)
{
    auto mq_cmd = std::make_unique<MockUndoableCommand>();
